...
const count = (target, input) => {
  return input.reduce( (ansSoFar, currentValue) => {
    ansSoFar = ansSoFar + currentValue.reduce((innerAnsSoFar, innerCurrentValue) => {
      if (target === innerCurrentValue){
        return innerAnsSoFar+1;
      }else {
        return innerAnsSoFar;
      }
    }, 0);
    return ansSoFar;
  }, 0);
...
const totalSum = (input) => {
  return input.reduce( (ansSoFar, currentValue) => {
    ansSoFar += currentValue.reduce((innerAnsSoFar, innerCurrentValue) => {
      return innerAnsSoFar + innerCurrentValue;
    }, 0);
    return ansSoFar;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input = input.map(element => {
    element = element.filter( number => {
      ...
      return Math.pow(2, filteredNumber);
    });
    return element;
  });
  return input;
...
let findMaleAndFemale = (data) => {
  ...
  ).reduce( (ansSoFar, currentValue, currentIndex) => {
    if (currentIndex === 0){
      return currentValue.name;
    } else {
      return ansSoFar + ' and ' + currentValue.name;
    }
  }, ''
  );
};
